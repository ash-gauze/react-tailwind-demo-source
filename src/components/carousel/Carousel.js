  ...
  return (
    <>
      <div style={scrollyStyle} className='flex [&::-webkit-slider-thumb]:bg-white rounded-t-lg flex-row flex-nowrap justify-around p-4 w-100vw bg-slate-600' >
        <div>        <button className='control-button rounded-s-xl w-12' onClick={() => handlePrevIndex()}>《</button></div>
      ...
      <div className='overflow-hidden my-8 carousel-viewport '>
        <div className='  carousel-collection flex flex-row flex-nowrap bg-slate-200 delay-200' style={{ transform: `translateX(${offset}px)` }} >

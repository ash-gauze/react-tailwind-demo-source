...
const Carousel = () => {
  ...
  const handleNextIndex = () => {
    if(offset === -totalLength - baseOffset) {
      setOffset(0)} else{
        setOffset(offset-baseOffset) }
    }
    /*
    this is bad syntax, linter does not approve 
    offset === -(totalLength - baseOffset) ? setOffset(0) : setOffset(offset - baseOffset);
    setMini(mini + 1)
    */
  const handlePrevIndex = ()=>{ 
  if(offset === 0) {setOffset(-(totalLength)-(baseOffset)) }
      else { setOffset(offset+baseOffset);
      }
    }
 /* 
  const handlePrevIndex = () => {
...
*/
//  const scrollyStyle = { overflowY:'scroll', msOverflowStyle: 'none', scrollbarWidth: 'none', '&WebkitScrollbar': { width: '0' }
 // }

  return (
    <>
      <div className={'scrollyStyle'}> 
